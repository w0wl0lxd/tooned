...
#[test]
fn config_init_default_uncommented_is_valid() {
    let dir = tempfile::tempdir().unwrap();
    let out = dir.path().join("tooned.toml");
    ...
    text = text.replace("# metrics_disabled = false", "metrics_disabled = false");
    text = text.replace("# precise_tokens = false", "precise_tokens = false");
