...
//! tier is therefore fully lossless -- [`expand_legend`] is a strict inverse
//! of [`apply_dict`] -- and is only ever applied when it strictly shrinks the
...
fn scan_structure(toon: &str) -> (Option<Vec<String>>, Vec<Vec<&str>>, Vec<(&str, &str)>) {
    ...
    if let Some(hi) = header_idx {
        let columns = parse_columns(lines[hi]);
...
fn transform_cells<'a>(
    ...
    if let Some(cols) = columns {
        for l in &lines {
            ...
            let cells = split_cells(l.trim_start());
...
mod tests {
    ...
    fn round_trips_array_with_repeated_values() {
        ...
        let dict = apply_dict(&toon, &[]).expect("should compress (repeated 'administrator')");
