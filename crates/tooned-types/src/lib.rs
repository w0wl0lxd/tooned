...
impl CriticalFieldPolicy {
    ...
    /// Check whether `key` (a column/key name) is protected by this policy.
    /// Matching is case-insensitive substring: if any protected substring
    /// appears in `key`, the key is protected.
    pub fn is_protected(&self, key: &str) -> bool {
        let lower = key.to_lowercase();
        self.protected.iter().any(|p| lower.contains(p))
    }
}
...
pub struct InspectReport {
    ...
    /// Column/key names protected from dictionary abbreviation by the
    /// critical-field policy (#3). Empty when no fields were protected or the
