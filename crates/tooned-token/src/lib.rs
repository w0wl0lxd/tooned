...
fn cl100k() -> &'static CoreBPE {
    ...
    SINGLETON.get_or_init(tiktoken_rs::cl100k_base_singleton)
}

/// Lazily-initialized `o200k_base` singleton (GPT-4o / o-series models).
fn o200k() -> &'static CoreBPE {
    static SINGLETON: OnceLock<CoreBPE> = OnceLock::new();
    SINGLETON.get_or_init(tiktoken_rs::o200k_base_singleton)
}
...
fn heuristic_count(text: &str) -> usize {
    ...
    // ceil(len / 4) without overflow or division-by-zero.
    (len + BYTES_PER_TOKEN - 1) / BYTES_PER_TOKEN
}
