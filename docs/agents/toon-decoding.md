...
This document shows that a model, given a TOON-encoded `additionalContext` from
a `PostToolUse` hook, decodes it into the underlying structure and answers as if
it had read the original JSON — no external decoder involved.
...
- **McMillan, 2026** — *Structured Context Engineering for File-Native
  ...
  significantly affect aggregate accuracy (chi-squared=2.45, p=0.484)" — direct
  evidence comprehension does not depend on the original JSON syntax.
- **Kutschka & Geiger, 2026** — *Notation Matters: A Benchmark Study of
  ...
  18% with accuracy within 9pp of JSON; largest savings on tool schemas and
  tool results — exactly where `tooned` injects TOON.
...
| CSV, TSV, nested JSON, large uniform JSON, plain text | `SKU-1001` | Decoded TOON, but *hedged* — reconciled against the original output |
...
context holds up; TOON as model output is more error-prone. `tooned` is built
around this — it only asks the model to *read* TOON, never to *write* it.

## Reproduce

The universal mismatch hook:
...
Point `.devin/hooks.v1.json`'s `PostToolUse` command at this script, run e.g.
`devin -p "read agent-test/records_20.xml and tell me the SKU of the first
product"`, then restore the real `tooned hook run --devin` entry.

## More

- Findings + reasoning traces: [`toon-evidence.md`](toon-evidence.md)
